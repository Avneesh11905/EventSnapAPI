...
async def test_sort_attendee_api(test_client, app_container):
    mock_use_case = AsyncMock()
    mock_use_case.execute.return_value = AttendeeSortDTO(event_code="EVT", matches_found=2, photos=["img1.jpg", "img2.jpg"])
...
async def test_generate_zip_api(test_client, app_container):
    ...
    with app_container.generate_zip_use_case.override(mock_use_case):
        response = await test_client.post(
            ...
            json={"event_id": "EVT", "user_id": "USER1", "image_paths": [{"url": "http", "key": "img1.jpg"}]}
        )
...
async def test_check_zip_api(test_client, app_container):
    mock_use_case = AsyncMock()
    mock_use_case.execute.return_value = ZipCheckDTO(exists=True, zip_path="path/to.zip", filename="to.zip")
