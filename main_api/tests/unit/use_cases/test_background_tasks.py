...
async def test_encode_image_batch_success():
    ...
    mock_storage.download_images.return_value = [
        "img1_data",
    ...
    mock_inference.get_face_encodings.return_value = [
        [
            {"embedding": [0.1] * 512, "confidence": 0.99},
            {"embedding": [0.2] * 512, "confidence": 0.85},
        ],  # img1 has 2 faces
        [],  # img3 has 0 faces
    ]
...
async def test_process_event_encoding_use_case():
    ...
    mock_storage.list_images.return_value = [
        "event/TEST/thumbs/img1.jpg",
        "event/TEST/thumbs/img2.jpg",
    ]
    mock_uow = MockUOW()
